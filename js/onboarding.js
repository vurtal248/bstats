...
export function initOnboarding(force = false) {
  ...
  function renderStep(initial = false) {
    ...
    const actualTtHeight =
      tooltip.offsetHeight > 50 ? tooltip.offsetHeight : 400;
    ...
    ttLeft = Math.max(
      safeMargin,
      Math.min(ttLeft, l - actualTtWidth - safeMargin),
    );
    ttTop = Math.max(
      safeMargin,
      Math.min(ttTop, t - actualTtHeight - safeMargin),
    );
    ...
    anime({
      ...
      left: 0,
      width: "100%",
      height: Math.max(0, t - hole.bottom),
      duration: dur,
      easing,
    });
